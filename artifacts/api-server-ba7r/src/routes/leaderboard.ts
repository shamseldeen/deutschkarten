...
router.post("/leaderboard/friend-group", requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!user?.friendCode) {
      ...
      await db.update(usersTable).set({ friendCode: code }).where(eq(usersTable.id, userId));
...
router.post("/leaderboard/join", requireAuth, async (req, res) => {
  ...
  try {
    ...
    await db.insert(friendGroupMembersTable).values({ groupId: group.id, userId }).onConflictDoNothing();
