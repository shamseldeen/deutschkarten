...
export default function ProfileTab() {
  ...
  if (!user) {
    return (
      ...
      >
        <Text
          style={[styles.title, { color: colors.foreground, marginBottom: 8 }]}
        >
    ...
    >
      ...
      >
        <Image
          ...
          style={{
            ...
            height: 88,
            borderRadius: 18,
            backgroundColor: rank.current.accent + "15",
          }}
        />
        <View style={{ flex: 1 }}>
          <Text
          ...
          <Text
            style={{ fontSize: 12, color: colors.foreground, marginTop: 4 }}
          >
            {rank.current.blurb}
          </Text>
          {rank.next ? (
                ...
                <Text style={{ fontSize: 11, color: colors.mutedForeground }}>
                  {rank.toNext > 0 ? (
                    ...
                    <Text
                      style={{ color: colors.foreground, fontWeight: "800" }}
                    >
                      Learn 1 C1 card → {rank.next.title}
                    </Text>
                  ) : (
                    <Text
                      style={{ color: colors.foreground, fontWeight: "800" }}
              ...
              >
                <View
                  style={{
                    ...
                    backgroundColor: rank.current.accent,
                  }}
                />
              </View>
            </View>
        ...
        >
          ...
          <View style={styles.statsRow}>
            <View style={styles.miniStat}>
              <Text style={[styles.miniNum, { color: colors.primary }]}>
                {quizStats.overall.accuracy}%
              </Text>
              <Text
            ...
            <View style={styles.miniStat}>
              <Text style={[styles.miniNum, { color: colors.foreground }]}>
                {quizStats.overall.sessions}
              </Text>
              <Text
                style={[styles.miniLabel, { color: colors.mutedForeground }]}
              >
          ...
          {quizStats.byMode.map((m) => (
            <View key={m.mode} style={{ marginTop: 10 }}>
              <View style={styles.modeRow}>
                ...
                <Text
                  style={[styles.modeRight, { color: colors.mutedForeground }]}
                >
              ...
              <View
                style={[styles.miniBarBg, { backgroundColor: colors.muted }]}
              >
                <View
                  style={[
                    styles.miniBarFill,
            ...
            return (
              ...
              >
                ...
                <View style={{ alignItems: "flex-end" }}>
                  <Text
                    style={[styles.historyPct, { color: colors.foreground }]}
                  >
                    {pct}%
                  </Text>
                  <Text style={{ fontSize: 11, color: colors.mutedForeground }}>
      ...
      {leaderboard && (leaderboard.top.length > 0 || leaderboard.me) && (
        ...
        >
          ...
          {leaderboard.me && (
            ...
            >
              <Text style={[styles.lbRank, { color: colors.foreground }]}>
                #{leaderboard.me.rank}
              </Text>
              <Text
                style={[styles.lbName, { color: colors.foreground, flex: 1 }]}
                numberOfLines={1}
              >
                You
              </Text>
              <Text style={[styles.lbXp, { color: colors.primary }]}>
                {leaderboard.me.xp.toLocaleString()} XP
              </Text>
            </View>
          )}
      ...
      {community && (
        ...
        >
          ...
          >
            Free for everyone. Every word added or translated is shared with all
            learners.
          </Text>
          <View style={styles.statsRow}>
            <View style={styles.miniStat}>
...
function LanguagesSection({
  ...
  const [editing, setEditing] = React.useState<"primary" | "secondary" | null>(
    null,
  );
  ...
  return (
    ...
    >
      ...
      >
        Pick which languages appear on each card. Translations are generated on
        demand and shared with everyone.
      </Text>

      <View style={styles.langRow}>
        <Pressable
          style={[
            styles.langPickerWrap,
            {
              borderWidth: 1,
        ...
        >
          <Text
            style={[styles.langPickerLabel, { color: colors.mutedForeground }]}
          >
            Primary
          </Text>
          <Text
          ...
          >
            {SUPPORTED_LANGS.find((l) => l.code === prefs.primaryLang)?.name ??
              prefs.primaryLang}
          </Text>
        </Pressable>
        <Pressable
          ...
          onPress={() =>
            setEditing(editing === "secondary" ? null : "secondary")
          }
        >
          <Text
            style={[styles.langPickerLabel, { color: colors.mutedForeground }]}
          ...
          >
            {prefs.secondaryLang
              ? (SUPPORTED_LANGS.find((l) => l.code === prefs.secondaryLang)
                  ?.name ?? prefs.secondaryLang)
              : "None"}
          </Text>
        </Pressable>
      </View>
      ...
      {editing && (
        <View style={styles.langOptions}>
          ...
          {SUPPORTED_LANGS.filter(
            (l) => editing === "primary" || l.code !== prefs.primaryLang,
          ).map((l) => {
            const isActive =
              editing === "primary"
                ? l.code === prefs.primaryLang
                : l.code === prefs.secondaryLang;
            return (
              <Pressable
                key={l.code}
                onPress={() => {
                  if (editing === "primary")
                    save({ ...prefs, primaryLang: l.code });
