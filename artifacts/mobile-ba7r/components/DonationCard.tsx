  ...
  return (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <View style={styles.row}>
        <Pressable
          onPress={() => open(5)}
          style={[styles.btn, { backgroundColor: "#ec489915", borderColor: "#ec489950" }]}
        >
          <View style={styles.btnHead}>
            <Feather name="heart" size={14} color="#ec4899" />
            <Text style={[styles.btnTitle, { color: colors.foreground }]}>Support Creator</Text>
          </View>
          <Text style={[styles.btnDesc, { color: colors.mutedForeground }]}>
            Thank the developer.
          </Text>
          <Text style={[styles.btnCta, { color: "#ec4899" }]}>Donate via PayPal →</Text>
        </Pressable>

        <Pressable
          onPress={() => open(10)}
          style={[styles.btn, { backgroundColor: "#3b82f615", borderColor: "#3b82f650" }]}
        >
          <View style={styles.btnHead}>
            <Feather name="server" size={14} color="#3b82f6" />
            <Text style={[styles.btnTitle, { color: colors.foreground }]}>Upgrade AI & Servers</Text>
          </View>
          <Text style={[styles.btnDesc, { color: colors.mutedForeground }]}>
            Faster AI, more storage.
          </Text>
          <Text style={[styles.btnCta, { color: "#3b82f6" }]}>Donate via PayPal →</Text>
        </Pressable>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  btnTitle: { fontSize: 14, fontWeight: "700", fontFamily: "Inter_600SemiBold" },
  btnDesc: { fontSize: 11 },
