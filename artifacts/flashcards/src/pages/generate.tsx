...
import { useGenerateFlashcards, FlashcardGenerateInputLevel } from "@workspace/api-client-react";
...
const generateSchema = z.object({
  level: z.enum([
    FlashcardGenerateInputLevel.A1, FlashcardGenerateInputLevel.A2,
    FlashcardGenerateInputLevel.B1, FlashcardGenerateInputLevel.B2,
...
export default function Generate() {
  ...
  const fetchStatus = useCallback(() => {
    fetch("/api/flashcards/generate/status")
      .then((r) => r.json())
  ...
  const form = useForm<GenerateFormValues>({
    ...
    defaultValues: { level: FlashcardGenerateInputLevel.A1, category: "", count: 5 },
  });

  const onSubmit = (data: GenerateFormValues) => {
    generateMutation.mutate({ data }, {
      onSuccess: (cards) => {
        toast({ title: "Cards Generated!", description: `Created ${cards.length} new flashcards.` });
        fetchStatus();
        setLocation("/browse");
      },
      onError: (error: any) => {
        ...
        if (error?.response?.status === 429 || body?.resetsAt) {
          ...
          setLimitStatus((prev) => prev ? { ...prev, remaining: 0, used: prev.limit } : null);
          toast({
            title: "Daily limit reached",
            description: "You've used all free AI generations for today. Keep learning in the meantime!",
            variant: "destructive",
          });
        } else {
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Use AI to instantly create rich, multi-lingual flashcards complete with context and examples.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-blue-200 bg-blue-50 px-5 py-4 flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800">
            ...
            {limitStatus ? (
              <>
                You have{" "}
                <span className={`font-bold ${limitStatus.remaining === 0 ? "text-red-600" : "text-blue-700"}`}>
                  {limitStatus.remaining} of {limitStatus.limit}
                </span>{" "}
                generations remaining today.
                {limitStatus.remaining === 0 && (
                  <> Resets in <span className="font-mono font-bold">{countdown || "calculating…"}</span>.</>
                )}
              </>
            ) : (
        ...
        {isBlocked ? (
          <Card className="border-red-200 bg-red-50">
            <CardContent className="pt-8 pb-8 flex flex-col items-center gap-4 text-center">
              ...
              <div>
                <p className="text-lg font-bold text-red-700">Daily limit reached</p>
              ...
              <div className="rounded-xl bg-white border border-red-200 px-8 py-4">
                <p className="text-xs text-red-400 uppercase font-semibold tracking-widest mb-1">Next generation available in</p>
                <p className="text-4xl font-mono font-black text-red-600">{countdown || "—"}</p>
              </div>
              <p className="text-sm text-muted-foreground max-w-sm">
                While you wait, keep studying your existing cards or browse all {" "}
                <button className="underline text-blue-600" onClick={() => setLocation("/browse")}>
                  your flashcards
                </button>.
              </p>
            </CardContent>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>Leave blank for random vocabulary for the selected level.</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                  <Button type="submit" size="lg" className="w-full font-bold" disabled={generateMutation.isPending}>
                    {generateMutation.isPending ? (
                      <><Loader2 className="w-5 h-5 mr-2 animate-spin" />Generating Magic…</>
                    ) : (
                      <><Wand2 className="w-5 h-5 mr-2" />Generate Cards</>
                    )}
                  </Button>
                </form>
              </Form>
