...
interface LimitStatus { limit: number; used: number; remaining: number; resetsAt: string; }
...
export default function GenerateScreen() {
  ...
  const fetchStatus = useCallback(() => {
    apiFetch(`/api/flashcards/generate/status`)
      .then((r) => r.json())
  ...
  const handleGenerate = () => {
    ...
    generate.mutate(
      ...
      {
        onSuccess: () => { setSuccess(true); fetchStatus(); },
        onError: (err: any) => {
  ...
  return (
    ...
    >
      ...
      {/* Blocked state */}
      {isBlocked ? (
        <View style={[styles.blockedCard, { backgroundColor: "#fef2f2", borderColor: "#fecaca" }]}>
          <Text style={styles.blockedIcon}>⏳</Text>
          <Text style={[styles.blockedTitle, { color: "#b91c1c" }]}>Daily limit reached</Text>
          <Text style={[styles.blockedSub, { color: "#dc2626" }]}>
            You've used all {limitStatus?.limit} free AI generations for today.
          </Text>
          <View style={styles.countdownBox}>
            <Text style={[styles.countdownLabel, { color: "#ef4444" }]}>Next generation in</Text>
            <Text style={[styles.countdownTimer, { color: "#b91c1c" }]}>{countdown || "—"}</Text>
          </View>
          <Text style={[styles.blockedHint, { color: appColors.mutedForeground }]}>
        ...
        <>
          ...
          <View style={styles.chipRow}>
            {LEVELS.map((lvl) => {
              ...
              return (
                <TouchableOpacity
                  key={lvl}
                  style={[styles.chip, { backgroundColor: active ? getLevelColor(lvl) : appColors.secondary, borderColor: active ? getLevelColor(lvl) : appColors.border }]}
                  onPress={() => setSelectedLevel(lvl as Level)}
                  testID={`select-level-${lvl}`}
                >
                  <Text style={[styles.chipText, { color: active ? "#fff" : appColors.foreground }]}>{lvl}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
          ...
          <View style={styles.chipRow}>
            {CATEGORIES.map((cat) => {
              ...
              return (
                <TouchableOpacity
                  key={cat}
                  style={[styles.chip, { backgroundColor: active ? appColors.primary : appColors.secondary, borderColor: active ? appColors.primary : appColors.border }]}
                  onPress={() => setSelectedCategory(cat)}
                  testID={`select-category-${cat}`}
                >
                  <Text style={[styles.chipText, { color: active ? "#fff" : appColors.foreground }]}>{cat}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
          ...
          <View style={styles.chipRow}>
            {COUNTS.map((c) => {
              ...
              return (
                ...
                >
                  <Text style={[styles.chipText, { color: active ? "#fff" : appColors.foreground }]}>{c}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
          ...
          >
            {generate.isPending ? <ActivityIndicator color="#fff" /> : (
          ...
          {success && (
            <View style={[styles.successMsg, { backgroundColor: "#dcfce7", borderColor: "#86efac" }]}>
              <Text style={styles.successText}>
                ✓ {selectedCount} new flashcards added! ({limitStatus?.remaining ?? "?"} generations left today)
              </Text>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  infoBanner: { borderRadius: 12, borderWidth: 1.5, padding: 12, marginBottom: 20, flexDirection: "row", gap: 8, alignItems: "flex-start" },
  ...
  blockedCard: { borderRadius: 16, borderWidth: 1.5, padding: 24, marginTop: 8, alignItems: "center", gap: 8 },
  blockedIcon: { fontSize: 40, marginBottom: 4 },
  blockedTitle: { fontSize: 20, fontWeight: "800" },
  blockedSub: { fontSize: 14, textAlign: "center" },
  countdownBox: { borderRadius: 12, backgroundColor: "#fff", borderWidth: 1, borderColor: "#fecaca", paddingHorizontal: 28, paddingVertical: 14, marginTop: 8, alignItems: "center" },
  countdownLabel: { fontSize: 11, fontWeight: "700", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 },
  countdownTimer: { fontSize: 36, fontWeight: "900", fontVariant: ["tabular-nums"] },
