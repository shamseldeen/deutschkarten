...
function LeaderRow({ row, highlight }: { row: Row; highlight?: boolean }) {
  return (
    <div className={
      ...
      (highlight ? "bg-cyan-50 dark:bg-cyan-950/30 ring-1 ring-cyan-300" : "hover:bg-slate-50 dark:hover:bg-slate-900")
    }>
      ...
      {row.imageUrl ? (
        ...
        <div className="w-9 h-9 rounded-full bg-cyan-100 dark:bg-cyan-900 flex items-center justify-center text-sm font-bold text-cyan-700 dark:text-cyan-300 shrink-0">
      ...
      <div className="text-right shrink-0">
        <div className="font-bold text-base text-cyan-600 dark:text-cyan-400">
...
function PersonalStats({ me }: { me: Row }) {
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {[
        { label: "XP Points", value: me.xp.toLocaleString(), icon: <Zap className="w-4 h-4 text-cyan-500" /> },
...
export default function LeaderboardPage() {
  ...
  const { data, isLoading } = useQuery<Resp>({
    ...
    queryFn: async () => {
      const r = await fetch(`${API}/leaderboard?mode=${mode === "personal" ? "public" : mode}`, { credentials: "include" });
  ...
  const { data: friendGroup } = useQuery<{ groupId: string; friendCode: string; memberCount: number }>({
    ...
    queryFn: async () => {
      const r = await fetch(`${API}/leaderboard/friend-group`, { method: "POST", credentials: "include" });
      if (!r.ok) throw new Error("failed");
      return r.json();
    },
  });
    ...
    <Layout>
      <div className="max-w-3xl mx-auto p-4 space-y-4">
          ...
          {tabs.map((t) => (
            <button
              ...
              className={
                ...
                (mode === t.key ? "bg-background shadow text-foreground" : "text-muted-foreground hover:text-foreground")
              }
            >
              {t.icon}{t.label}
            </button>
        ...
        {mode === "personal" && !me && !isLoading && (
          <Card className="p-6 text-center text-slate-500 text-sm">Sign in to see your personal stats.</Card>
        )}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    maxLength={8}
                  />
                  <Button size="sm" disabled={joinCode.length < 6 || joinMutation.isPending} onClick={() => joinMutation.mutate(joinCode)}>
                    Join
                  </Button>
                </div>
              </div>
          ...
          <Card className="border-2 border-cyan-400">
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-cyan-500" />Your rank
              </CardTitle>
            </CardHeader>
            <CardContent><LeaderRow row={me} highlight /></CardContent>
          </Card>
        )}
            ...
            <CardContent className="space-y-1">
              ...
              {!isLoading && top.length === 0 && (
                <div className="text-sm text-slate-500 py-6 text-center">
                  {mode === "friends" ? "No friends yet — share your invite code to start competing!" : "No one has earned XP yet — be the first!"}
                </div>
              )}
              {top.map((r) => <LeaderRow key={r.userId} row={r} highlight={r.userId === myId} />)}
            </CardContent>
          </Card>
        )}
      </div>
