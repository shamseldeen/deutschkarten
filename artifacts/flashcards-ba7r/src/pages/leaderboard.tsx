...
export default function LeaderboardPage() {
  ...
  const { data, isLoading } = useQuery<Resp>({
    ...
    queryFn: async () => {
      const r = await fetch(`${basePath}/api/leaderboard`, { credentials: "include" });
  ...
  return (
    <div
      className={
        ...
        (highlight ? "bg-yellow-50 dark:bg-yellow-950/30" : "hover:bg-slate-50 dark:hover:bg-slate-900")
      }
    >
      <div className="w-6 flex justify-center">{rankBadge(row.rank)}</div>
      {row.imageUrl ? (
        <img src={row.imageUrl} alt="" className="w-8 h-8 rounded-full object-cover" />
      ) : (
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-slate-500 flex gap-3">
          <span className="flex items-center gap-1"><BookOpen className="w-3 h-3" />{row.knownCards}</span>
          <span className="flex items-center gap-1"><Sparkles className="w-3 h-3" />{row.correctAnswers}</span>
          <span className="flex items-center gap-1"><Flame className="w-3 h-3" />{row.longestStreak}</span>
        </div>
      </div>
      <div className="font-bold text-lg text-yellow-600 dark:text-yellow-400">{row.xp.toLocaleString()}<span className="text-xs ml-1 font-normal text-slate-500">XP</span></div>
    </div>
  );
}
