...
import { Brain, Trophy, Target, Activity, Languages, Sparkles, Type } from "lucide-react";
...
export default function StatsPage() {
    ...
    return (
      <Layout>
        <div className="max-w-md mx-auto py-16 text-center space-y-4">
          ...
          <p className="text-muted-foreground">Track quiz history, accuracy, and progress.</p>
  ...
  const overall = stats?.overall ?? { sessions: 0, questions: 0, correct: 0, accuracy: 0 };
    ...
    <Layout>
      <div className="max-w-4xl mx-auto space-y-8">
        <div>
          ...
          <p className="text-muted-foreground">How your German is coming along.</p>
        </div>
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><Activity className="w-5 h-5" />By Mode</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            {!stats?.byMode.length && (
              <p className="text-sm text-muted-foreground text-center py-6">
                No quizzes yet. <Link href="/quiz"><span className="text-primary font-semibold hover:underline">Start your first quiz →</span></Link>
              </p>
            )}
            {stats?.byMode.map((m) => {
              const meta = MODE_META[m.mode] ?? { label: m.mode, icon: Brain, color: "text-foreground" };
              ...
              return (
                <div key={m.mode} className="space-y-1.5">
                  <div className="flex justify-between items-center text-sm">
                    ...
                    <span className="text-muted-foreground">{m.correct}/{m.questions} · <span className="font-bold text-foreground">{m.accuracy}%</span></span>
                  </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {!history?.length && (
              <p className="text-sm text-muted-foreground text-center py-6">No completed quizzes yet.</p>
            )}
            {history?.map((row) => {
              const meta = MODE_META[row.mode] ?? { label: row.mode, icon: Brain, color: "text-foreground" };
              const Icon = meta.icon;
              const pct = row.totalQuestions > 0 ? Math.round((row.correctAnswers / row.totalQuestions) * 100) : 0;
              return (
                <div key={row.id} className="flex items-center justify-between p-3 rounded-lg border border-border hover-elevate">
                  <div className="flex items-center gap-3">
                    <Icon className={`w-5 h-5 ${meta.color}`} />
                    <div>
                      <div className="font-semibold text-sm">{meta.label}{row.level ? ` · ${row.level}` : ""}</div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground">{row.correctAnswers}/{row.totalQuestions}</div>
                  </div>
                </div>
              );
            })}
