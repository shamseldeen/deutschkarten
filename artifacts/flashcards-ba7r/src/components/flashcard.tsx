...
export function Flashcard({
  ...
  const [card, setCard] = useState<CardWithTranslations>(
    incomingCard as CardWithTranslations,
  );
      ...
      >
        ...
        <div className="absolute inset-0 w-full h-full backface-hidden rounded-xl bg-card flex flex-col p-6 overflow-hidden">
          <div className="flex justify-between items-start mb-auto">
            <Badge
              variant="outline"
              className={cn("font-bold text-sm", getLevelColor(card.level))}
            >
              {card.level}
            </Badge>
            <div className="flex gap-1.5">
              ...
              {card.createdBy && (
                <Badge
                  ...
                  title="Contributed by the community"
                >
            ...
            <div className="flex flex-col items-center gap-3">
              <h2 className="text-4xl md:text-5xl font-black text-foreground font-serif tracking-tight">
                {card.article && (
                  <span
                    className={cn(
                      "mr-2 font-normal opacity-80 text-3xl md:text-4xl",
              ...
              >
                <Volume2
                  className={cn("w-3.5 h-3.5", isSpeaking && "animate-pulse")}
                />
          ...
          <div className="flex flex-col gap-5 flex-1">
            {langList.map((lang) => {
              ...
              return (
                <div
                  key={lang}
                  className={cn("space-y-1", rtl && "text-right")}
                  dir={rtl ? "rtl" : "ltr"}
                >
                  <h3 className="text-xs uppercase tracking-widest text-muted-foreground font-semibold">
                    {meta?.name ?? lang}{" "}
                    {meta && meta.nativeName !== meta.name
                      ? `(${meta.nativeName})`
                      : ""}
                  </h3>
                  <p
